...
# 1) 브랜드, 여행, 티켓, 등산, 낚시, 배낭, 유아, 아동, 운동, 여가, 중성, 혼용, 공용, 유니섹스, 남녀 등 (※ 스포츠는 배제 단어에서 제외)
# 2) 골프, 안전, 구기, 꽃, 생필품, 배달음식, 수입명품, 가구, DIY, PC, 주변기기, 가공식품 등 (※ 생활용품은 배제 단어에서 제외)
# 3) 임부복, 임산부, 마터니티 등 임부 관련 카테고리는 매핑에서 완전 제외
...
def forbidden_words_for(filter_name: str) -> tuple[str, ...]:
    """필터명(및 동의어)에 없는 절대 배제 단어 목록을 구한다 (필터명/동의어에 있으면 배제 대상에서 제외)."""
    filter_norm = normalize(filter_name)
    parsed = parse_filter_name(filter_name)
    all_terms = expand_synonyms([parsed.raw, parsed.top, parsed.mid, *parsed.lows])
    combined_norm = filter_norm + "".join(normalize(t) for t in all_terms)
    return tuple(w for w in FORBIDDEN_BASE_WORDS if normalize(w) not in combined_norm)


def is_forbidden_category(path: str, filter_name: str) -> bool:
    """카테고리 경로가 필터명/동의어에 없는 절대 배제 단어를 포함하는가."""
...
def strip_forbidden_categories(paths: Sequence[str], filter_name: str) -> list[str]:
    """필터명/동의어에 없는 절대 배제 단어가 들어간 카테고리를 완전히 제거한다."""
    forbidden = forbidden_words_for(filter_name)
    if not forbidden:
        return list(paths)
...
# ★요건 2026-08-23 확장:
# 1. 자켓 = 재킷 (상호 동의어)
# 6. 패딩 = 점퍼 = 패딩점퍼 = 야상 = 패딩다운 = 사파리 = 바람막이 (유사동의어)
# 7. 가디건 = 스웨터 = 외출복 (유사동의어)
# 8. 정장 = 수트 (동일어)
...
ITEM_SYNONYMS: dict[str, tuple[str, ...]] = {
    # 모자 = 야구모자 = 스포트모자 = 스포츠모자
    "모자": ("모자", "야구모자", "스포트모자", "스포츠모자", "볼캡", "캡모자", "캡", "햇"),
    "야구모자": ("야구모자", "스포트모자", "스포츠모자", "모자", "볼캡", "캡모자", "캡"),
    "스포트모자": ("스포트모자", "스포츠모자", "야구모자", "모자", "볼캡", "캡모자", "캡"),
    "스포츠모자": ("스포츠모자", "스포트모자", "야구모자", "모자", "볼캡", "캡모자", "캡"),
    "캡모자": ("캡모자", "볼캡", "야구모자", "스포트모자", "스포츠모자", "모자", "캡"),
    # 비니 = 머리수건 = 햇 = 햇볕가리개 = 햇빛가리개
    "비니": ("비니", "머리수건", "햇", "햇볕가리개", "햇빛가리개", "니트모자", "털모자", "방한모"),
    "머리수건": ("머리수건", "비니", "햇", "햇볕가리개", "햇빛가리개", "두건", "모자"),
    # 사파리 = 햇빛가리개 = 햇 = 낚시모자
    "햇": ("햇", "사파리", "사파리햇", "햇빛가리개", "햇볕가리개", "낚시모자", "비니", "머리수건", "버킷햇", "모자"),
    "햇볕가리개": ("햇볕가리개", "햇빛가리개", "사파리", "사파리햇", "햇", "낚시모자", "비니", "머리수건", "썬캡", "모자"),
    "햇빛가리개": ("햇빛가리개", "햇볕가리개", "사파리", "사파리햇", "햇", "낚시모자", "비니", "머리수건", "썬캡", "모자"),
    "낚시모자": ("낚시모자", "사파리", "사파리햇", "햇빛가리개", "햇볕가리개", "햇", "등산모자", "버킷햇", "모자"),
    "사파리": ("사파리", "사파리햇", "사파리모자", "햇빛가리개", "햇볕가리개", "햇", "낚시모자", "야상", "점퍼", "패딩", "패딩점퍼", "바람막이", "아우터", "자켓", "재킷"),
    "사파리햇": ("사파리햇", "사파리", "사파리모자", "햇빛가리개", "햇볕가리개", "햇", "낚시모자", "버킷햇", "벙거지", "모자"),
    "사파리모자": ("사파리모자", "사파리햇", "사파리", "햇빛가리개", "햇볕가리개", "햇", "낚시모자", "버킷햇", "모자"),
    "바라클라바": ("바라클라바", "방한모", "발라클라바", "마스크모자"),
    "버킷햇": ("버킷햇", "벙거지", "버킷", "사파리햇", "햇"),
    "선글라스": ("선글라스", "썬글라스", "아이웨어"),
    "안경테": ("안경테", "안경", "아이웨어"),
    "스니커즈": ("스니커즈", "운동화", "캔버스화"),
    # 11. 슬리퍼 = 슬립온 = 플립플랍 = 수영장 샌들 = 비치 신발
    "슬리퍼": ("슬리퍼", "슬립온", "플립플랍", "수영장샌들", "비치신발", "수영장 샌들", "비치 신발", "비치슈즈", "샌들", "쪼리"),
    "슬립온": ("슬립온", "슬리퍼", "플립플랍", "수영장샌들", "비치신발", "수영장 샌들", "비치 신발", "스니커즈"),
    "플립플랍": ("플립플랍", "슬리퍼", "쪼리", "수영장샌들", "비치신발", "수영장 샌들", "비치 신발", "샌들"),
    "수영장샌들": ("수영장샌들", "수영장 샌들", "비치신발", "비치 신발", "슬리퍼", "플립플랍", "샌들"),
    "수영장 샌들": ("수영장 샌들", "수영장샌들", "비치 신발", "비치신발", "슬리퍼", "플립플랍", "샌들"),
    "비치신발": ("비치신발", "비치 신발", "수영장샌들", "수영장 샌들", "슬리퍼", "플립플랍", "아쿠아슈즈"),
    "비치 신발": ("비치 신발", "비치신발", "수영장 샌들", "수영장샌들", "슬리퍼", "플립플랍", "아쿠아슈즈"),
    # 9 & 10 & 12. 단화 = 하이힐(미들) = 하이힐(로우) = 뮬 = 블로퍼 = 캐쥬얼구두 등
    "구두": ("구두", "단화", "정장구두", "로퍼", "캐쥬얼구두", "캐주얼구두", "옥스퍼드", "옥스포드", "하이힐", "뮬", "블로퍼", "드레스슈즈"),
    "단화": ("단화", "구두", "하이힐(미들)", "하이힐(로우)", "미들힐", "로우힐", "하이힐", "뮬", "블로퍼", "로퍼", "정장구두", "캐쥬얼구두", "캐주얼구두", "옥스퍼드", "옥스포드", "플랫슈즈"),
    "하이힐": ("하이힐", "하이힐(미들)", "하이힐(로우)", "미들힐", "로우힐", "단화", "뮬", "블로퍼", "구두", "펌프스"),
    "하이힐(미들)": ("하이힐(미들)", "미들힐", "하이힐(로우)", "로우힐", "하이힐", "단화", "뮬", "블로퍼", "구두"),
    "하이힐(로우)": ("하이힐(로우)", "로우힐", "하이힐(미들)", "미들힐", "하이힐", "단화", "뮬", "블로퍼", "구두"),
    "미들힐": ("미들힐", "하이힐(미들)", "로우힐", "하이힐(로우)", "단화", "뮬", "블로퍼", "하이힐"),
    "로우힐": ("로우힐", "하이힐(로우)", "미들힐", "하이힐(미들)", "단화", "뮬", "블로퍼", "하이힐"),
    "정장구두": ("정장구두", "로퍼", "구두", "단화", "드레스슈즈"),
    "로퍼": ("로퍼", "정장구두", "단화", "구두"),
    # 9 & 10. 캐쥬얼구두 = 옥스퍼드, 뮬 = 블로퍼 = 캐쥬얼구두
    "옥스퍼드": ("옥스퍼드", "옥스포드", "캐쥬얼구두", "캐주얼구두", "케쥬얼구두", "뮬", "블로퍼", "단화", "구두"),
    "옥스포드": ("옥스포드", "옥스퍼드", "캐쥬얼구두", "캐주얼구두", "케쥬얼구두", "뮬", "블로퍼", "단화", "구두"),
    "뮬": ("뮬", "블로퍼", "단화", "하이힐(미들)", "하이힐(로우)", "미들힐", "로우힐", "하이힐", "캐쥬얼구두", "캐주얼구두", "케쥬얼구두", "옥스퍼드", "옥스포드", "슬리퍼"),
    "블로퍼": ("블로퍼", "뮬", "단화", "하이힐(미들)", "하이힐(로우)", "미들힐", "로우힐", "하이힐", "캐쥬얼구두", "캐주얼구두", "케쥬얼구두", "옥스퍼드", "옥스포드", "슬리퍼"),
    "캐쥬얼구두": ("캐쥬얼구두", "캐주얼구두", "케쥬얼구두", "뮬", "블로퍼", "단화", "하이힐(미들)", "하이힐(로우)", "옥스퍼드", "옥스포드", "로퍼", "구두"),
    "캐주얼구두": ("캐주얼구두", "캐쥬얼구두", "케쥬얼구두", "뮬", "블로퍼", "단화", "하이힐(미들)", "하이힐(로우)", "옥스퍼드", "옥스포드", "로퍼", "구두"),
    "케쥬얼구두": ("케쥬얼구두", "캐쥬얼구두", "캐주얼구두", "뮬", "블로퍼", "단화", "하이힐(미들)", "하이힐(로우)", "옥스퍼드", "옥스포드", "로퍼", "구두"),
    # 1. 자켓 = 재킷
    "자켓": ("자켓", "재킷", "아우터", "점퍼", "바람막이"),
    "재킷": ("재킷", "자켓", "아우터", "점퍼", "바람막이"),
    # 8. 정장 = 수트
    "정장": ("정장", "수트", "슈트", "정장세트"),
    "수트": ("수트", "슈트", "정장", "정장세트"),
    "슈트": ("슈트", "수트", "정장"),
    # 6. 패딩 = 점퍼 = 패딩점퍼 = 야상 = 패딩다운 = 사파리 = 바람막이
    "패딩": ("패딩", "점퍼", "패딩점퍼", "야상", "패딩다운", "다운점퍼", "사파리", "바람막이", "아우터"),
    "점퍼": ("점퍼", "패딩", "패딩점퍼", "야상", "패딩다운", "다운점퍼", "사파리", "바람막이", "아우터", "자켓", "재킷"),
    "패딩점퍼": ("패딩점퍼", "패딩", "점퍼", "야상", "패딩다운", "다운점퍼", "사파리", "바람막이", "아우터"),
    "야상": ("야상", "점퍼", "패딩", "패딩점퍼", "사파리", "바람막이", "아우터"),
    "패딩다운": ("패딩다운", "다운점퍼", "패딩", "점퍼", "패딩점퍼", "아우터"),
    "바람막이": ("바람막이", "점퍼", "자켓", "재킷", "사파리", "아우터", "윈드브레이커"),
    # 7. 가디건 = 스웨터 = 외출복
    "가디건": ("가디건", "스웨터", "외출복", "카디건", "니트"),
    "카디건": ("카디건", "가디건", "스웨터", "외출복", "니트"),
    "스웨터": ("스웨터", "가디건", "카디건", "외출복", "니트"),
    "외출복": ("외출복", "스웨터", "가디건", "니트", "상의", "원피스"),
    "니트": ("니트", "스웨터", "가디건", "카디건", "외출복"),
    "가방": ("가방", "백팩", "크로스백", "토트백"),
    "지갑": ("지갑", "카드지갑", "머니클립"),
    "목도리": ("목도리", "머플러", "스카프"),
    "장갑": ("장갑", "글러브", "핸드워머"),
...
def find_category(
    ...
    *,
    ...
    for round_no in range(1, MAX_DB_ROUNDS + 1):
        # 2) 하위 카테고리로 엑셀에서 검색: 원문 키워드 우선 -> 가까운 동의어 순차 매칭
        exact_hits = filter_paths(paths, low_terms)
        if exact_hits:
            low_hits = exact_hits
        else:
            low_hits = []
            for syn in expand_synonyms(low_terms):
                syn_hits = filter_paths(paths, [syn])
                if syn_hits:
                    low_hits = syn_hits
                    break
            if not low_hits:
