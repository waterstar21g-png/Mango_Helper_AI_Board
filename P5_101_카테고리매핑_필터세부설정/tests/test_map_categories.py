...
def test_map_one_market_touches_mango_exactly_once(monkeypatch):
    ...
    def fake_once(
        ...
        *,
        ...
        cat, _ = mc.best_category_with_step(name, cats, exclude=exclude, region_type=region_type)
        calls.append(cat)
...
def test_map_one_market_blocks_opposite_gender(monkeypatch):
    ...
    def fake_best(name, cats, *, exclude=(), region_type="국내", db=None, keyword_db=None, ext_db=None, **kwargs):
        calls["n"] += 1
        if calls["n"] == 1:
