...
| 작업표시줄에 고정만 | `망고보드_작업표시줄고정.bat` 더블클릭 |
| 파이썬 직접 | `py -3 board\desktop_icon.py` (`--pin-only` 로 고정만) |

- **실행파일 아이콘 1개만** 만듭니다 — `망고보드.lnk` → `run.bat` (작업 폴더 = 망고보드 루트).
  타깃은 `cmd.exe /c "…\run.bat"` 입니다. `.bat` 을 직접 가리키면 Windows 가
  **작업표시줄 고정을 막기** 때문입니다.
- **작업표시줄 고정**은 아이콘 생성 시 자동으로 시도합니다. Windows 가 자동 고정을
  막으면(Win10 1903+ · Win11) 아이콘이 선택된 탐색기 창을 띄우므로, 우클릭 →
  [작업 표시줄에 고정] 을 누르면 됩니다 (Win11 은 [자세히 보기] 안).
...
| P3_설정수정_카테고리매핑초기화 | `P3_설정수정_카테고리매핑초기화/` | 지정 행 범위의 카테고리매핑 설정 일괄 초기화 |
...
| `망고보드_작업표시줄고정.bat` | 바탕화면 아이콘을 작업표시줄에 고정 |
